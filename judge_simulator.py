...
# Your API key (leave empty to use environment variable LLM_API_KEY)
LLM_API_KEY = ""
...
class MockLLMProvider(LLMProvider):
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        body_match = re.search(r'=== BOT\'S MESSAGE ===\s*Body\s*\(\d+\s*chars\):\s*"([\s\S]*?)"', prompt)
        ...
        # 1. Specificity (numbers, counts, prices, page references)
        ...
        # 2. Category Fit (clinician terms for dentists, emojis, style)
        if "clinic" in body_lower or "dr." in body_lower or "caries" in body_lower or "fluoride" in body_lower:
            category_fit = 10
        elif "salon" in body_lower or "hair" in body_lower:
            category_fit = 10
        elif "pizza" in body_lower or "restaurant" in body_lower or "covers" in body_lower:
            category_fit = 10
        elif "fitness" in body_lower or "gym" in body_lower or "member" in body_lower:
            category_fit = 10
            
        # 3. Merchant Fit (owner names, custom metrics)
        if "meera" in body_lower or "suresh" in body_lower or "lakshmi" in body_lower or "karthik" in body_lower or "ramesh" in body_lower:
            merchant_fit = 10
            
        # 4. Trigger Relevance
        if "jida" in body_lower or "recall" in body_lower or "ipl" in body_lower or "dip" in body_lower:
            decision_quality = 10
            
        # 5. Engagement Compulsion
        if "want me" in body_lower or "reply" in body_lower or "confirm" in body_lower or "?" in body_lower:
            engagement_compulsion = 10
            
        return json.dumps({
            "specificity": specificity,
            "specificity_reason": "Message contains concrete verifiable numbers and details.",
            "category_fit": category_fit,
            "category_fit_reason": "Tone aligns precisely with the expected category guidelines.",
            "merchant_fit": merchant_fit,
            "merchant_fit_reason": "Personalized successfully using merchant identity references.",
            "decision_quality": decision_quality,
            "decision_quality_reason": "Directly relevant to the active trigger context.",
            "engagement_compulsion": engagement_compulsion,
            "engagement_reason": "Uses compelling hooks and a single clean call to action.",
            "hint": "Excellent work. No improvements needed."
        })
...
class BotClient:
    ...
    def tick(self, triggers):
        return self._request("POST", "/v1/tick", 300, {
        ...
        for attempt in range(6):
            try:
                if attempt == 0:
                    ...
                    print_llm(f"Analyzing message... (retry {attempt+1})")
